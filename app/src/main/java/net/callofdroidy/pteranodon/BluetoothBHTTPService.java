 ...
 * It has
 *     a thread(AcceptThread) for listening for incoming connections,
 *     a thread(ConnectedThread) for performing data transmissions when connected.
 */
public class BluetoothBHTTPService {
    // Debugging
    private static final String TAG = "BluetoothBHTTPService";

    // Name for the SDP record when creating server socket
    private static final String NAME = "Pteranodon";
     ...
     */
    public BluetoothBHTTPService(Context context, Handler handler) {
    ...
    /**
     * Start the Bluetooth HTTP service. Specifically start AcceptThread to begin a session in listening (server) mode.
     * Called by the Activity onResume()
     */
    public synchronized void start() {
        Log.d(TAG, "start");

        // enable Bluetooth adapter and make it always discoverable
        if(mAdapter != null){
            if(!mAdapter.isEnabled())
                mAdapter.enable();
            context.startActivity(new Intent(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE).putExtra(BluetoothAdapter.EXTRA_DISCOVERABLE_DURATION, 0).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
        }
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            while (mState != STATE_CONNECTED) {
                ...
                if (socket != null) {
                    synchronized (BluetoothBHTTPService.this) {
                        switch (mState) {
                            case STATE_LISTEN:
